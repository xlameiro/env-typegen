---
title: Overview
description: env-typegen turns .env.example files into typed artifacts and enforces environment contracts with pull, diff, and verify governance workflows.
---

env-typegen combines code generation and governance in one workflow:

- Generate typed artifacts from `.env.example`
- Pull provider state through adapters
- Validate real environments against a contract
- Detect drift before production deploys
- Enforce CI gates with `verify`
...
| Contract + drift (`verify`) | Deterministic CI gate (warning/error => fail)     |
...
# CI gate
env-typegen verify --env .env --contract env.contract.mjs
```
...
| Understand the product vision  | [Manifesto](/docs/manifesto)                |
