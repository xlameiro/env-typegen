...
- **Use CVA (`class-variance-authority`) for any component that accepts `variant` or `size` props** — export the variants function (e.g., `buttonVariants`, `badgeVariants`) so consumers can reuse the styles without rendering the component. Type props via `VariantProps<typeof myVariants>` — never define manual variant union types.
- **`ButtonLink` for link-as-button pattern** — when a navigation link must look like a button, use `<ButtonLink href="...">` from `@/components/ui/button-link`. It renders a semantic `<a>` (via Next.js `<Link>`) with full `buttonVariants` styling. Never render `<button disabled tabIndex={0}>` as a navigation anchor — that is a semantic and accessibility violation.
