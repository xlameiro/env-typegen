...
- **300-line ceiling per file** — no source file may exceed 300 non-blank, non-comment lines (enforced by the `max-lines` ESLint rule). This is not arbitrary: LLMs retrieve and reason over individual files as atomic units; a file that exceeds this length cannot be processed in full within a single reasoning pass, which degrades completion quality, review accuracy, and refactor safety. At ~250 meaningful lines, look for natural seams to split: extract a sub-component, move pure helpers to a sibling `*.utils.ts`, or separate `page.tsx` + `actions.ts` + `queries.ts`. Blank lines and JSDoc/inline comments do not count toward this limit — they are documentation, not logic density. Config files (`*.config.*`) are exempt.
